build-all-yield-features:
	...
	@mv target/deploy/dynamic_ext.so target/deploy/no-yield.so
	anchor build -p dynamic_ext -- --features transfer-hook,permissioned-wrapping,no-yield --no-default-features
