...
impl Deposit<'_> {
    ...
    pub fn handler(ctx: Context<Self>, amount: u64) -> Result<()> {
        ...
        // Mint receipt tokens to depositer
        let cpi_context = CpiContext::new_with_signer(
            ctx.accounts.lp_receipt_token_program.to_account_info(),
            MintToChecked {
                mint: ctx.accounts.lp_receipt_mint.to_account_info(),
                to: ctx.accounts.actor_receipt_token_account.to_account_info(),
                authority: ctx.accounts.pool.to_account_info(),
            },
            &seeds,
        );
        mint_to_checked(cpi_context, amount, ctx.accounts.lp_receipt_mint.decimals)?;
        ...
        let cpi_context = CpiContext::new(
            ...
            TransferChecked {
                ...
                from: ctx.accounts.actor_token_account.to_account_info(),
