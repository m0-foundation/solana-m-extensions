...
impl ClaimFees<'_> {
    pub fn handler(ctx: Context<Self>) -> Result<()> {
        ...
        // Mint new tokens to the recipient if there are any fees to claim
        let accrued_fee_principal = ctx.accounts.global_account.accrued_fee_principal;
        if accrued_fee_principal > 0 {
            mint_tokens(
                &ctx.accounts.recipient_ext_token_account,
                accrued_fee_principal,
            ...
            // Reset the accrued fee principal after minting
            ctx.accounts.global_account.accrued_fee_principal = 0;

            emit!(FeesClaimed {
                recipient_token_account: ctx.accounts.recipient_ext_token_account.key(),
                amount: principal_to_amount_down(accrued_fee_principal, multiplier)?,
                principal: accrued_fee_principal,
            });
        }
